<xsl:stylesheet version="1.0"
  ...
  <xsl:function name="htmlbook:is-chunk">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$node[self::h:section[contains(@data-type, 'acknowledgments') or
		      ...
		      contains(@data-type, 'toc')]]">1</xsl:when>
      <xsl:when test="$node[self::h:nav[contains(@data-type, 'toc')]]">1</xsl:when>
	...
	<xsl:if test="($sect-level = '1' or $sect-level = '2' or $sect-level = '3' or $sect-level = '4' or $sect-level = '5') and
		      $sect-level &lt;= $chunk.level">1</xsl:if>	
      </xsl:when>
    </xsl:choose>
  </xsl:function>
  ...
  <xsl:function name="htmlbook:chunk-for-node">
	...
	<xsl:sequence select="$chunk.node"/>
      </xsl:when>
    </xsl:choose>
  </xsl:function>

  <xsl:function name="htmlbook:section-depth">
    <xsl:param name="node"/>
    <xsl:choose>
